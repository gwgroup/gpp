...
var insertToken = function (user_id, cb) {
    ...
    Redis.set(`master_token_${token}`, { user_id, token }, ExpiresConfig.token);
    cb(undefined, token);
};
...
var checkToken = function (token, cb) {
    Redis.get(`master_token_${token}`, (err, result) => {
        if (err) {
            return cb(err);
        }
        if (result) {
...
var delay = function (token) {
    Redis.flushExpire(`master_token_${token}`,ExpiresConfig.token);
}
